...
if "raw" in args.methods:
    print("RAW ENSEMBLE")
    X_raw_trainval = np.load(os.path.join(root_ref, "X_raw_trainval.npy")).astype(np.float32)
    X_raw_test = np.load(os.path.join(root_ref, "X_raw_test.npy")).astype(np.float32)
    trainval_dict["raw_ensemble"] = [X_raw_trainval,"Raw Ensemble"]
    test_dict["raw_ensemble"] = [X_raw_test,"Raw Ensemble"]
    del X_raw_trainval, X_raw_test
...
if "qrf" in args.methods:
    ...
    qrf_trainval = np.load(os.path.join(root_ref,best_qrf_trainval[:3]+"_pred"+best_qrf_trainval[3:])).astype(np.float32)
    qrf_test = np.load(os.path.join(root_ref,best_qrf_test[:3]+"_pred"+best_qrf_test[3:])).astype(np.float32)
    trainval_dict[".".join(best_qrf_trainval.split('.')[:-1])] = [qrf_trainval,"QRF"]
    test_dict[".".join(best_qrf_trainval.split('.')[:-1])] = [qrf_test,"QRF"]
...
for distrib in ["gtcnd","csgd"]:
    if f"qrf+{distrib}" in args.methods:
        ...
        qrftail_trainval = np.load(os.path.join(root_ref,best_qrftail_trainval[:(4+len(distrib))]+"_pred"+best_qrftail_trainval[(4+len(distrib)):])).astype(np.float32)
        qrftail_test = np.load(os.path.join(root_ref,best_qrftail_test[:(4+len(distrib))]+"_pred"+best_qrftail_test[(4+len(distrib)):])).astype(np.float32)
